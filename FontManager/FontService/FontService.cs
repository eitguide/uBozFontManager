...
namespace FontManager.FontService
{
    internal class FontService : IDisposable
    {
        ...
        //internal virtual Bitmap RenderCharacterCode(long unicode)
        //{
        ...
        private class DebugChar
        {
            ...
            internal DebugChar(uint c, float advanceX, float bearingX, float width)
            {
        ...
        public static SolidBrush ColorSelected = new SolidBrush(ColorHelper.ConvertToARGB("#bdc3c7"));
        public static SolidBrush ColorDisable = new SolidBrush(ColorHelper.ConvertToARGB("#ecf0f1"));
        public static SolidBrush ColorStringDisable = new SolidBrush(ColorHelper.ConvertToARGB("#34495e"));
        ...
        public static void DrawFontItem(DrawItemEventArgs e, FontInfo fontInfo, ListBox lb)
        {
            ...
            else
            {
                if (fontInfo.Disable)
                {
                    e.Graphics.FillRectangle(ColorDisable, e.Bounds);
                }
                else
                {
            ...
            if (fontInfo.Disable == true)
            {
                e.Graphics.DrawString(fontInfo.NameInRegistry, e.Font, ColorStringDisable, titleBounds, _fmt);
        ...
        public Bitmap RenderCharacterCode(uint code)
        {
            return RenderCharacterCode(this.lib, this.FontFace, code, SystemColors.ControlText, Color.Transparent);
        }

        internal static Bitmap RenderCharacterCode(Library library, Face face, uint code, Color foreColor, Color backColor)
        {
            var measuredChars = new List<DebugChar>();
            var renderedChars = new List<DebugChar>();
            float penX = 0, penY = 0;
